...
class CaseCreationWrapper:
    ...
    async def after_call(self, msg: Msg, result: Msg) -> None:
        if self.state.get("case_type") != "" and self.state.get("case_number") == "":
...
class EscalationAssessmentWrapper:
    ...
    async def after_call(self, msg: Msg, result: Msg) -> None:
        if self.state.get("escalation_tier") != "" and self.state.get("escalation_required") == False:
...
class CaseResolutionWrapper:
    ...
    async def after_call(self, msg: Msg, result: Msg) -> None:
        if self.state.get("case_description") != "" and self.state.get("case_resolved") == False:
            self.state.set("escalation_score", self.state.get("escalation_score") + 20)
...
CRITICAL: Whenever the user provides any of the following values — customer_email, customer_name, case_type, case_description — you MUST immediately call _set_variables_customer_verification(customer_email=<value>, customer_name=<value>, case_type=<value>, case_description=<value>) to save them before calling any other tool. Do NOT skip this step."""
...
class AgentBot:
    ...
    def _build_agents(self):
        ...
        async def _set_variables_customer_verification(customer_email: str | None = None, customer_name: str | None = None, case_type: str | None = None, case_description: str | None = None):
